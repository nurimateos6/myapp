...
def get_recipe(request: Request):
    conn = get_recipes()
    len_recipes = len(conn)
    return templates.TemplateResponse('recipes/recipes.html', {'request': request, 'recipes': conn, 'len': len_recipes})
...
def get_recipe_user(username: str, request: Request):
    ...
    return templates.TemplateResponse('recipes/recipesprivate.html',
                                      {'request': request, 'recipes': conn, 'len': len_recipes, 'username': username})
...
def get_recipe_form(username: str, request: Request):
    return templates.TemplateResponse('recipes/recipes_form.html', {'request': request, 'username': username})
...
async def create_recipe(request: Request):
    ...
    if await recipe.is_valid():
        ...
        try:
            if res.is_insert:
                return conn.execute(recipes.select().where(recipes.c.recipe_id == res.inserted_primary_key[0])).first()
